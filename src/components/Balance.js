...
const Balance = () => {
	...
	useEffect(() => {
		if (exchange && tokens[0] && tokens[1] && account)
			// verify if everything is loaded first
			loadBalances(dispatch, exchange, account, tokens);
	}, [exchange, tokens, account, transferInProgress, dispatch]); // run the useEffect when any of this variables change
		...
		<div className='component exchange__transfers'>
			...
			<div className='exchange__transfers--form'>
				...
				<form onSubmit={transferToken1Handler}>
					<label htmlFor='token0'>{symbols ? symbols[0] : null} Amount</label>
					<input
						type='text'
						id='token0'
						placeholder='0.0000'
			...
			<div className='exchange__transfers--form'>
				...
				<form onSubmit={transferToken2Handler}>
					<label htmlFor='token1'>{symbols ? symbols[1] : null} Amount</label>
					<input
						type='text'
						id='token1'
						placeholder='0.0000'
